...
export class FlawList {
  ...
  protected add(): void { if (this.form.invalid) return; const value = this.form.getRawValue(); this.service.addFlaw(value.title.trim(), value.description.trim()); this.form.reset({ title: '', description: '' }); }
  protected remove(id: string): void { this.service.removeFlaw(id); }
  ...
  protected import(event: Event): void { const input = event.target as HTMLInputElement; const file = input.files?.[0]; if (!file) return; const reader = new FileReader(); reader.onload = () => { try { this.importError = !this.service.importFlaws(JSON.parse(String(reader.result))); } catch { this.importError = true; } input.value = ''; }; reader.readAsText(file); }
}